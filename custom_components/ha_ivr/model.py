...
# מאיזה אורך רצף ספרות נקרא ספרה-ספרה. מספרי טלפון בישראל הם 9
# עד 10 ספרות ומספרי הזמנה 6 עד 8, ואילו שנה, מחיר וטמפרטורה הם
# ארבע ספרות ומטה — ולכן שבע היא הגבול שמפריד ביניהם בלי לפגוע
# בכמויות אמיתיות.
SPELL_FROM = 7

_LONG_RUN = re.compile(rf"\d{{{SPELL_FROM},}}")


def spell_long_digits(text: str) -> str:
    """ריווח רצף ספרות ארוך, כדי שיוקרא ספרה-ספרה.

    מנוע הקראה קורא `0501234567` כסכום — "חמש מיליארד..." — ולא
    כמספר טלפון. רווח בין הספרות הוא מה שכל מנוע מבין, ואינו
    דורש ממנו יכולת מיוחדת.

    חל על טקסט חופשי בלבד: מספר שהליבה מייצרת עובר דרך
    `say_number`, שכבר יודע להגיש אותו כראוי.
    """
    return _LONG_RUN.sub(lambda m: " ".join(m.group()), str(text))
